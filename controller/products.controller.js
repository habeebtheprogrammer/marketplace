...
const { productsService, usersService, vendorsService, promoService } = require("../service")
...
exports.getProducts = async (req, res, next) => {
    ...
    try {
        ...
        sort = sort == 'highToLow' ? { sort: { ...searchSortObj, original_price: -1 } } : sort == 'lowToHigh' ? { sort: { ...searchSortObj, original_price: 1 } } : { sort: { ...searchSortObj, updatedAt: -1 } };
        ...
        if (query.slug && data.totalDocs == 1) {
            ...
            if(applied && (data.docs[0]._id?.toString() == obj.rewards.productId._id?.toString())){
