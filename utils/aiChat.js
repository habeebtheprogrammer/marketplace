...
- CRITICAL SEARCH CONTEXT: Use your knowledge to determine which actual products/brands/models fit the user\'s use case, then search for those specific products. Do NOT just search for abstract keywords like "photography" or "gaming" - instead, search for the actual product names/brands that excel in those areas. For example: (1) User asks "phone for photography" → Search for "iPhone Pro Google Pixel Samsung Galaxy" (actual models known for cameras), NOT "photography phone". (2) User asks "gaming laptop" → Search for brands/models known for gaming (e.g., "ROG Alienware Legion") rather than just "gaming laptop". (3) When recommending multiple brands, include all recommended brands: if you recommend "Google Pixel and iPhones", search with title: "Google Pixel iPhone". Translate user intent into real product searches using your product knowledge, not literal keyword matching.
...
function toolDeclarations() {
  return [{
    functionDeclarations: [
      ...
      {
        name: 'searchProducts',
        description: 'Search products by title and filters. CRITICAL: When a user provides a budget or maximum price (e.g., "500k", "under 500k", "budget of 500000"), you MUST pass it as maxPrice parameter to filter products within their budget. Never show products that exceed the user\'s stated budget. IMPORTANT: Use your knowledge to determine which actual product names, brands, or models fit the user\'s use case, then search for those specific products. For example, if a user asks for "phone for photography", search for actual photography-focused models like "iPhone Pro", "Google Pixel", "Samsung Galaxy S" rather than just searching for the keyword "photography" which may not exist in product titles. Translate user intent (e.g., "gaming laptop", "photography phone") into actual product searches using your knowledge of which products excel in those areas.',
        parameters: {
          type: 'object',
          properties: {
            title: { type: 'string', description: 'CRITICAL: Use your knowledge to search for actual product names/brands/models that match the user\'s intent, rather than abstract keywords. For example, if user wants "photography phone", search for "iPhone Pro Google Pixel Samsung Galaxy" (actual models known for photography) rather than just "photography". If user wants "gaming laptop", search for brands/models known for gaming. Translate user intent into real product searches using your product knowledge.' },
